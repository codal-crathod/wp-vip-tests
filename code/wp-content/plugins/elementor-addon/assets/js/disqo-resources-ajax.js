...
jQuery(document).ready(function($) {
	...
	function ajax_call( pagelink='', pagenumb = 1, isPushState = 1 ) {
		...
		$.ajax({
			url: disqo_resources.ajaxurl,
			type: 'post',
			data: {
				action: 'ajax_disqo_resources',
				pagelink: pagelink,
				pagenum: pagenumb,
				per_page: $('#resources-page').data("per-page"),
				by_tag: $('#resources-page').data("by-tag"),
				ex_tag: $('#resources-page').data("ex-tag"),
				title: $('#resources-page').data("title"),
				page_id: $('#resources-page').data("page-id"),
				featured_blogid: $('[data-elementor-type=wp-page] #featured-block').data("tagid"),
				filter: filter,
			},
			beforeSend: function() {
				/*$('html, body').animate({
